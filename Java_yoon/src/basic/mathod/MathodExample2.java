...
() 내부를 비워둡니다. 그리고 호출할 때도 () 내부를 비워서 호출합니다. 
*/

public class MathodExample2 {

	//x부터 y까지의 누적합계를 구하는 메서드
	static int calcRangeTotal(int start, int end) {//매개변수는 255개까지
		int total = 0;
	...
	//매개변수가 필요 없는 경우
	static String selectRandomFood() { //괄호를 비워두더라도 있어야 한다. 아니면 변수 선언으로 받아들여진다.
	...
	//정수 n개를 전달하면 그 정수들의 총합을 리턴하는 메서드
...
*/
	...
	//가변인수(가변 파라미터)를 사용한 매개 변수
	//콤마로 나열되어 들어오는 여러개의 값을 배열로 묶어서 내부로 전달합니다.
	static int calcNumberTotal2(int... nums) {//중복 메서드는 선언할 수 없다(이름)
		int total = 0;
		for(int i : nums) {
			total += i;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("오늘 점심 뭐 먹지?: " + selectRandomFood());
